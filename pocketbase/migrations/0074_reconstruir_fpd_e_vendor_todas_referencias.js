...
/**
 * Migration 0074: Reconstruir fpd_records e vendor_consolidations para TODAS as referências
 * com paginação completa (batch 2000 em loop até esgotar), deduplicação canônica fiel e isolamento estrito.
 */
migrate(
  (app) => {
    ...
    console.log('[Migration 0074] Carregando lojas cadastradas...')
    var storesCollection = app.findCollectionByNameOrId('stores')
    var allStores = []
    try {
      allStores = app.findRecordsByFilter('stores', '1=1', 'name', 500, 0)
    } catch (errStores) {
      console.log('[Migration 0074] Erro ao carregar stores:', errStores)
    }
    ...
    console.log('[Migration 0074] Lendo todas as linhas de movel...')
    ...
    if (app.hasTable('movel')) {
      ...
      while (true) {
        ...
        try {
          ...
          console.log('[Migration 0074] Erro ao paginar movel:', eM)
          break
        }
      ...
      console.log('[Migration 0074] Total linhas brutas lidas de movel: ' + totalMovelRead)
    }

    console.log('[Migration 0074] Lendo todas as linhas de residencial...')
    ...
    if (app.hasTable('residencial')) {
      ...
      while (true) {
        ...
        try {
          ...
          console.log('[Migration 0074] Erro ao paginar residencial:', eR)
          break
        }
      ...
      console.log('[Migration 0074] Total linhas brutas lidas de residencial: ' + totalResRead)
    }
    ...
    console.log(
      '[Migration 0074] Referências encontradas para reconstrução: ' +
        JSON.stringify(allTargetRefs),
    )

    // Limpar fpd_records e vendor_consolidations completamente
    console.log('[Migration 0074] Limpando fpd_records e vendor_consolidations...')
    try {
      app.db().newQuery('DELETE FROM fpd_records').execute()
      app.db().newQuery('DELETE FROM vendor_consolidations').execute()
    } catch (eDel) {
      console.log('[Migration 0074] Erro ao limpar tabelas consolidadas:', eDel)
    }
    ...
    for (var refIdx = 0; refIdx < allTargetRefs.length; refIdx++) {
      ...
      var processRecord = function (row) {
        ...
        // Determinar supervisão/coordenação conforme cadastro das lojas
      ...
      for (var sIdx = 0; sIdx < storeIds.length; sIdx++) {
        ...
        try {
          ...
          console.log('[Migration 0074] Erro ao salvar fpd_record:', eSFpd)
        }
      }
      ...
      for (var vIdx = 0; vIdx < vendorKeys.length; vIdx++) {
        ...
        try {
          ...
          console.log('[Migration 0074] Erro ao salvar vendor_consolidation:', eSVend)
        }
      }
    ...
    console.log(
      '[Migration 0074] Finalizada com sucesso! Total consolidado geral: fpd_records = ' +
        grandTotalFpd +
